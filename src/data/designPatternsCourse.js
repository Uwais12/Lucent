        ...
        // ---------------------------
        {
          ...
          parts: [
            ...
            },
            {
              ...
              content:
                "**Decorator Pattern:** Attaches additional responsibilities to an object dynamically. Decorators provide a flexible alternative to subclassing for extending functionality.\n\n**UML-like Structure:**\n```\n+------------------+\n|    Component     |<-----------------+\n|------------------|                  |\n| + operation()    |                  |\n+------------------+                  |\n     ^         ^                      |\n     |         |                      |\n+---------+ +------------------+      |\n|Concrete | |    Decorator     |<>----+\n|Component| |------------------|\n+---------+ | - component      |\n            | + operation()    |\n            +------------------+\n                 ^         ^\n                 |         |\n          +----------+ +----------+\n          |ConcreteA | |ConcreteB |\n          |Decorator | |Decorator |\n          +----------+ +----------+\n```\n\n* **Key Idea:** Decorators have the same supertype as the objects they decorate (allowing them to be used interchangeably) and wrap the object they decorate (composition).\n* **Benefits:**\n    * Flexibility: Add/remove responsibilities at runtime.\n    * Avoids feature-laden superclasses or class explosion.\n    * Follows Open/Closed Principle: Extend behavior without modifying existing component code.\n* **Drawbacks:** Can lead to many small objects, potentially complex to configure.\n* **Real-World Examples:**\n    * Java I/O classes (`FileInputStream` decorated by `BufferedInputStream`, then maybe `DataInputStream`).\n    * Python's `@decorator` syntax for wrapping functions with additional behavior (logging, authentication, caching).\n    * Express.js middleware: each middleware wraps the request handler with additional behavior (CORS, authentication, compression).\n    * gzip compression in HTTP servers: response streams are wrapped with compression decorators.\n\n* **Common Pitfalls:**\n    * Type identity breakage: `instanceof` checks against the original type fail when objects are wrapped in decorators.\n    * Ordering sensitivity: The order in which decorators are applied can matter (e.g., encrypt-then-compress vs. compress-then-encrypt).\n    * Debugging difficulty: Deep decorator chains can make stack traces hard to read.\n    * Decorator vs. inheritance confusion: Use decorators for optional, composable additions; use inheritance for fundamental 'is-a' relationships.",
              order: 3,
              duration: 15,
               exercise: {
                 type: "multiple-choice",
        ...
        { // Was L1 in original template's Ch1
          ...
          parts: [
            ...
            },
            {
              ...
              content:
                "While simple, the Singleton pattern is sometimes considered an anti-pattern if overused.\n\n* **Valid Use Cases:** Logging frameworks, configuration managers, thread pools, hardware interface access where only one instance makes sense.\n* **Concerns:**\n    * **Global State:** Introduces global state, making code harder to test and reason about.\n    * **Tight Coupling:** Clients become directly coupled to the concrete Singleton class.\n    * **Violates Single Responsibility Principle?** The class manages its own lifecycle *and* does its main job.\n    * **Testing:** Difficult to mock or replace the Singleton instance in unit tests.\n\n* **Real-World Examples:**\n    * `Runtime.getRuntime()` in Java -- only one JVM runtime per application.\n    * `window` object in browsers -- a single global context.\n    * Database connection pools (e.g., HikariCP) are typically singletons.\n    * Spring Framework beans are singletons by default within their container.\n\n* **Common Pitfalls:**\n    * **Hidden dependencies:** Code that calls `MySingleton.getInstance()` anywhere has a hidden dependency that is invisible in constructors/method signatures, making the dependency graph unclear.\n    * **Initialization order:** In languages with static initialization, the order of singleton creation across classes can cause subtle bugs.\n    * **Serialization/Reflection:** In Java, serialization and reflection can break the singleton guarantee if not handled carefully (use `readResolve()` or enum-based singletons).\n\nConsider alternatives like **Dependency Injection** before resorting to Singleton. Modern frameworks (Spring, Guice, Dagger) manage singleton scope for you without the pattern's drawbacks.",
              order: 4,
              duration: 5,
        ...
        // ---------------------------
        {
          ...
          parts: [
            ...
            },
            {
              ...
              content:
                "**Command Pattern:** Encapsulates a request as an object, thereby letting you parameterize clients with different requests, queue or log requests, and support undoable operations.\n\n* **Intent:** Decouple the sender/invoker of a request from the receiver of the request.\n* **Benefits:**\n    * Decouples invoker from receiver.\n    * Commands are first-class objects (can be stored, passed around, queued).\n    * Easy to add new commands without changing invoker.\n    * Supports undo/redo functionality.\n    * Can assemble composite commands (macros).\n* **Uses:** GUI button actions, undo/redo, transaction logging, queuing work, macro recording.\n\n* **Real-World Examples:**\n    * Redux actions in React applications are essentially Command objects dispatched to a store.\n    * Git commits can be viewed as Commands -- each encapsulates a change that can be applied (cherry-pick) or reversed (revert).\n    * Database transactions: each operation is a command that can be committed or rolled back.\n    * Java's `Runnable` interface is a simple form of Command.\n\n* **Common Pitfalls:**\n    * Complex undo logic: For non-trivial commands, storing enough state to undo correctly can be challenging (consider using the Memento pattern alongside Command).\n    * Command bloat: Creating a separate class for every tiny action can lead to many small classes. Consider whether a simpler callback or lambda suffices.\n    * Memory leaks: Storing command history indefinitely for undo can consume significant memory -- implement a bounded history.",
              order: 3,
              duration: 15,
              exercise: {
                 type: "multiple-choice",
        ...
        // ---------------------------
        {
          ...
          parts: [
            ...
            },
            {
              ...
              content:
                "**State Pattern:** Allows an object to alter its behavior when its internal state changes. The object will appear to change its class.\n\n* **Intent:** Encapsulate state-specific behavior and manage state transitions.\n* **Benefits:**\n    * Localizes state-specific behavior into separate classes.\n    * Makes state transitions explicit.\n    * Adheres to Open/Closed Principle (adding new states often involves adding new classes, not modifying existing ones).\n* **Relation to Strategy:** Structure is similar (Context delegates to another object). State manages transitions *between* different behavior objects; Strategy usually lets the client choose the behavior object.\n\n* **Real-World Examples:**\n    * TCP connection states (Listen, SynSent, Established, Closed) -- each state handles packets differently.\n    * UI component states in React (loading, error, success) -- each renders differently.\n    * Order processing in e-commerce (Pending, Paid, Shipped, Delivered, Cancelled).\n    * Media player states (Playing, Paused, Stopped) -- button behavior changes based on current state.\n\n* **Common Pitfalls:**\n    * State explosion: Too many states with too many transitions can make the system hard to understand. Consider a state machine diagram before coding.\n    * Transition ownership ambiguity: Decide clearly whether the Context or the State objects manage transitions -- mixing approaches creates confusion.\n    * Shared state: If state objects need to share data, careful design is needed to avoid tight coupling between state classes.",
              order: 3,
              duration: 15,
               exercise: {
                 type: "multiple-choice",
